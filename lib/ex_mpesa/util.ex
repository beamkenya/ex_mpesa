defmodule ExMpesa.Util do
  @moduledoc false

  @doc false
  @spec get_url(String.t(), String.t()) :: String.t()
  def get_url(live_url, sandbox_url) do
    if Application.get_env(:ex_mpesa, :sandbox) == false, do: live_url, else: sandbox_url
  end
  ...
  def get_security_credential_for(key) do
    securityCredential = Application.get_env(:ex_mpesa, key)[:security_credential]

    cond do
      securityCredential === "" || securityCredential === nil ->
        cert = Application.get_env(:ex_mpesa, :cert)
        password = Application.get_env(:ex_mpesa, key)[:password]
